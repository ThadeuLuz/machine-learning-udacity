...
class Sigmoid:
    ...
    def update(self, values, train, eta=.1):
        ...
        for X, y_true in zip(values, train):
            ...
            update = eta * accuracy * derivative * X
            self.weights += update
...
def test():
    ...
    assert sum_almost_equal(u1.weights, np.array(
        [2.990752, -2.018496, 0.972257]))

    u2 = Sigmoid(weights=[0, 3, -1])
    u2.update(np.array([[-3, -1, 2], [2, 1, 2]]), np.array([1, 0]))
    assert sum_almost_equal(u2.weights, np.array(
        [-0.030739, 2.984961, -1.027437]))
